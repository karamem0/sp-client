...
Describe 'Add-SPClientContentTypeField' {
        ...
        Context 'Site Content Type' {
            ...
            AfterEach {
                try {
                    ...
                    $SPClient.ClientContext.Load($ContentType.FieldLinks)
                    $SPClient.ClientContext.ExecuteQuery()
                    for ($index = $ContentType.FieldLinks.Count - 1; $index -ge 0; $index--) {
                        $FieldLink = $ContentType.FieldLinks[$index]
                        $SPClient.ClientContext.Load($FieldLink)
                        $SPClient.ClientContext.ExecuteQuery()
                        if ($FieldLink.Name -eq 'TestField0') {
            ...
            It 'Adds a field to the content type' {
                $Web = $SPClient.ClientContext.Site.OpenWebById($SPClient.TestConfig.WebId)
                $ContentType = $Web.ContentTypes.GetById($SPClient.TestConfig.WebContentTypeId)
                $Field = $Web.Fields.GetByInternalNameOrTitle('TestField0')
                $Params = @{
                ...
                $Result = Add-SPClientContentTypeField @Params
